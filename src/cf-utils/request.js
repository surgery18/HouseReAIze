...
- Characters must not talk over each other
- Ensure buffer time between dialogues to prevent characters from talking over each other
- Try the dialoge times roughly how long it would be in real life but with that added buffer time mentioned above
...
- Make sure the episode timeline flows right. After one event is done, it should go right to the next. No pause in between
- All characters in a scene MUST be defined in the initial state. A character cannot just suddenly appear if they are not defined in the initial state
...
- The last event's timestamp and duration must add up to the episode's total duration
...
By default, the episode should revolve around a rare and challenging medical case unless the user specifies otherwise.

## Characters

These are the only available characters to use: House, Cuddy, Wilson, Chase, Cameron, and a Patient. You don't have to use all the characters. They are just available to use. However, House must always be in the script, and the patient must be in the script.
...
- **timeline:** This is a list detailing what happens at specific times in the episode. Each event in the timeline has:
  ...
  - **action:** What is happening at that timestamp. The action has an action_type which can be one of the following:
    ...
    - **talk:** A character speaks.
      ...
      - duration: Duration in seconds for how long this dialogue will be displayed or spoken, with added buffer time to prevent characters from talking over each other.
    ...
    - **rotate:** When a character rotates to a specific angle.
    ...
    - **scene:** Represents a change in the scene or setting.
